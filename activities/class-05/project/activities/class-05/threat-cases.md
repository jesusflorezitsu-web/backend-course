...
3. Lectura de ticket ajeno: un requester hace `GET /requests/:id` con el id de otro requester → **404 RESOURCE_NOT_FOUND**, idéntico a un id inexistente. Nunca se revela si el recurso existe.
...
5. PATCH con body mixto: un requester envía `{ "title": "Nuevo", "priority": "high" }` sobre su propia solicitud → **403 ACTION_NOT_ALLOWED** y NO se aplica el título (todo-o-nada).
6. Edición de contenido fuera de "abierta": el dueño patchea el título de su solicitud en otro estado → **403 ACTION_NOT_ALLOWED** (`canEditContent` exige `open`).
7. Sin credencial o con Bearer vacío: `GET /requests` sin header `Authorization` (o `Bearer ` vacío) → **401 UNAUTHORIZED**. El middleware corta antes de que llegue al router.
...
15. Cabecera de autorización rara: `Authorization: Basic dXNlcjpwYXNz` → **401 UNAUTHORIZED**. Solo el esquema Bearer acredita identidad aquí.